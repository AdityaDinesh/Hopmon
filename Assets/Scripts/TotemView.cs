...
public class TotemView : MonoBehaviour
{
    ...
    private void RotateBody()
    {
        ...
        if (currentDirection == Vector3.zero || _bodyTransform == null) return;

        // Axis perpendicular to movement direction
        Vector3 rotationAxis = Vector3.Cross(-currentDirection, Vector3.up);

        // Calculate how much to rotate this frame
        float rotationAmount = speed * 90f * Time.deltaTime;
